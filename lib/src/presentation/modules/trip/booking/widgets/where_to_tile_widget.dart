...
class _WhereToTileWidget extends ConsumerWidget {
  ...
  Future<void> _pickByMap(
    ...
    WidgetRef ref,
    LocationPointDetail? destination,
  ) async {
    LocationPointDetail? result;
    ...
    if (context.router.current.name == TripRoute.name) {
      result = await context.router.replace<LocationPointDetail?>(
  ...
  Future<void> _pickBySearch(
    ...
    Option<LocationPointDetail>? result;

    /// If this is not the root page we do replace.
    if (context.router.current.name == TripRoute.name) {
      result = await context.router.push<Option<LocationPointDetail>?>(route);
    } else {
      result =
          await context.router.replace<Option<LocationPointDetail>?>(route);
    }
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final destination = ref.watch(
      bookingController.select(
        (state) => state.expectedEndedPositionDetail.fold(() => null, id),
      ),
    );

    return InkWell(
      onTap: () => _pickBySearch(context, ref, destination),
      child: Container(
